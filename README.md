...
> **This fork extends the original scDEAL with a SMILES-based multi-drug model.**
> See [SMILES Extension](#smiles-extension) below for what changed and how to run it.
> Everything else in this document describes the original single-drug scDEAL pipeline
> (`bulkmodel.py` / `scmodel.py`), which is still present and usable as-is.
...
This fork adds a **multi-drug** variant of scDEAL that integrates the chemical
structure of each drug (as a SMILES string) into the model, instead of training
one independent model per drug as the original pipeline does.
...
In addition to `data/ALL_expression.csv` and `data/ALL_label_binary_wf.csv`
(same as the original pipeline), the SMILES extension requires:
...
documented above (`--lr`, `--epochs`, `--bottleneck`, `--dropout`,
`--encoder_h_dims` / `--bulk_h_dims`, `--predictor_h_dims`, `--dimreduce`,
`--device`, `--checkpoint`, `--sampling`, `--var_genes_disp`, `--min_g`,
`--min_c`, `--mmd_weight`, `--mmd_GAMMA`, `--cluster_res`, etc.):
